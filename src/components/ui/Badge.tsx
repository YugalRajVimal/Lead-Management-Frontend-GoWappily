...
import { LeadStatus, Priority, SourceSyncStatus, ProjectStatus, TaskStatus } from "@/lib/types";

const statusStyles: Record<LeadStatus, string> = {
  new: "bg-blue-50 text-blue-700 ring-blue-600/20",
  "not contacted": "bg-amber-50 text-amber-700 ring-amber-600/20",
  "call later": "bg-purple-50 text-purple-700 ring-purple-600/20",
  pitched: "bg-teal-50 text-teal-700 ring-teal-600/20",
  "quotation send": "bg-cyan-50 text-cyan-700 ring-cyan-600/20",
  "follow up": "bg-indigo-50 text-indigo-700 ring-indigo-600/20",
...
export function TaskStatusBadge({ status }: { status: TaskStatus }) {
  return <Base className={taskStatusStyles[status]}>{status.replace("_", " ")}</Base>;
}
