...
export function Header({ title }: { title: string }) {
  ...
  return (
    <header className="flex h-14 items-center justify-between border-b border-slate-200 bg-white px-4 md:px-6 shrink-0">
      <h1 className="text-sm font-semibold text-[#173F76]">{title}</h1>
      <div className="relative" ref={ref}>
        <button
          onClick={() => setOpen((o) => !o)}
          className="relative rounded-md p-2 text-slate-500 hover:bg-[#EAF3FC] hover:text-[#1C6FC9] transition-colors"
        >
          <Bell className="h-4.5 w-4.5" />
          {unread > 0 && (
            <span className="absolute top-1 right-1 flex h-4 min-w-4 items-center justify-center rounded-full bg-[#F5871F] px-1 text-[10px] font-semibold text-white">
              {unread}
            </span>
          )}
        </button>
          ...
          <div className="absolute right-0 mt-2 w-80 rounded-lg border border-slate-200 bg-white shadow-lg z-20">
            <div className="flex items-center justify-between px-3.5 py-2.5 border-b border-slate-100">
              ...
              <Link
                href="/notifications/"
                onClick={() => setOpen(false)}
                className="text-xs text-[#1C6FC9] hover:text-[#173F76]"
              >
                View all
              </Link>
            </div>
              ...
              {notifications.slice(0, 8).map((n) => {
                ...
                return (
                  ...
                  >
                    ...
                    {!n.read && (
                      <span className="h-1.5 w-1.5 rounded-full bg-[#F5871F] mt-1 shrink-0" />
                    )}
                  </Link>
                );
              })}
