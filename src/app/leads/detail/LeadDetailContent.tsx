...
export function LeadDetailContent() {
  ...
  const addConversation = async (text: string) => {
    if (!lead) return;
    try {
      const entry = await api.addLeadConversation(lead._id, text);
      setLead({ ...lead, conversationLog: [...(lead.conversationLog ?? []), entry] });
    } catch (err) {
      toast(err instanceof ApiError ? err.message : "Failed to log conversation", "error");
    }
  };

  const editConversation = async (entryId: string, text: string) => {
    if (!lead) return;
    try {
      const updated = await api.updateLeadConversation(lead._id, entryId, text);
      setLead({
        ...lead,
        conversationLog: (lead.conversationLog ?? []).map((c) =>
          c._id === entryId ? updated : c
        ),
      });
    } catch (err) {
      toast(err instanceof ApiError ? err.message : "Failed to save edit", "error");
    }
  };
  ...
  const deleteConversation = async (entryId: string) => {
    if (!lead) return;
    try {
      await api.deleteLeadConversation(lead._id, entryId);
      setLead({
        ...lead,
        conversationLog: (lead.conversationLog ?? []).filter((c) => c._id !== entryId),
      });
    } catch (err) {
      toast(err instanceof ApiError ? err.message : "Failed to delete entry", "error");
    }
  };
  ...
  return (
    <div className="p-4 md:p-6  space-y-5">
      ...
      {/* Phase 4 — Conversation Log. Same card styling as the sections above
          (rounded-xl/border-slate-100/shadow-sm, #0B2C5F heading) so it
          matches the rest of this page, but with its own subtitle so it
          isn't mistaken for "Project Details" (internal notes) above. */}
      <div className="rounded-xl border border-slate-100 bg-white shadow-sm p-4">
        <h3 className="text-sm font-semibold text-[#0B2C5F]">Conversation Log</h3>
        <p className="text-xs text-slate-400 mb-3">
          Recorded conversations with this lead — calls, WhatsApp, email, in-person.
        </p>
        <ConversationLog
          entries={lead.conversationLog ?? []}
