...
const STATUSES: ProjectStatus[] = ["active", "on_hold", "completed", "cancelled"];
type Tab = "overview" | "doclinks" | "team" | "tasks" | "conversations";
...
export function ProjectDetailContent() {
  ...
  const addConversation = async (text: string) => {
    if (!project) return;
    try {
      const entry = await api.addProjectConversation(project._id, text);
      setProject({
        ...project,
        conversationLog: [...(project.conversationLog ?? []), entry],
      });
    } catch (err) {
  ...
  const editConversation = async (entryId: string, text: string) => {
    if (!project) return;
    try {
      const updated = await api.updateProjectConversation(project._id, entryId, text);
      setProject({
        ...project,
        conversationLog: (project.conversationLog ?? []).map((c) =>
  ...
  const deleteConversation = async (entryId: string) => {
    if (!project) return;
    try {
      await api.deleteProjectConversation(project._id, entryId);
      setProject({
        ...project,
        conversationLog: (project.conversationLog ?? []).filter((c) => c._id !== entryId),
      });
    } catch (err) {
      ...
      <div className="flex items-center gap-1 border-b border-slate-200">
        {(
          [
            ...
            ["team", `Team (${project.teamMembers.length})`],
            ["tasks", "Tasks"],
            ["conversations", `Conversation Log (${project.conversationLog?.length ?? 0})`],
          ] as [Tab, string][]
        ).map(([key, label]) => (
          <button
            key={key}
      ...
      {tab === "conversations" && (
        <div className="rounded-lg border border-slate-200 bg-white p-5">
          ...
          <p className="text-xs text-amber-600 mb-3">
            Admin-only / internal — not visible to the client.
          </p>
          <ConversationLog
            entries={project.conversationLog ?? []}
