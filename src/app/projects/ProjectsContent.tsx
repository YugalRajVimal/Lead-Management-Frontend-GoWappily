...
export function ProjectsContent() {
  ...
  // Preset via ?teamMember=<id> — used by the Team page's "View projects" links (Epic 4).
  ...
  const load = useCallback(() => {
    ...
    const query: ProjectsQuery = {
      ...
      ...(tag ? { tag } : {}),
      ...(teamMemberFilter ? { teamMember: teamMemberFilter } : {}),
    };
    api
      .getProjects(query)
      .then((res) => {
  ...
  }, [page, search, status, tag, teamMemberFilter]);
  ...
  useEffect(() => {
    setPage(1);
  }, [search, status, tag, teamMemberFilter]);
  ...
  const hasFilters = status || tag || search || teamMemberFilter;
  const clearFilters = () => {
    setSearchInput("");
    setStatus("");
    setTag("");
  ...
  const filteredMemberName = teamMemberFilter
    ? users.find((u) => u.id === teamMemberFilter)?.name
    : null;
  ...
  return (
    ...
    <div className="p-4 md:p-6 space-y-4">
        ...
        {filteredMemberName && (
          <div className="flex items-center gap-2 rounded-md bg-indigo-50 px-3 py-2 text-xs text-indigo-700">
            <span>
              Showing projects for <strong>{filteredMemberName}</strong>
            </span>
            <button
              onClick={() => setTeamMemberFilter("")}
              className="ml-auto flex items-center gap-1 text-indigo-500 hover:text-indigo-800"
            >
