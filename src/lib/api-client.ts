...
// ---------------- Phase 4: Conversation Log (Leads) ----------------
// Additive — mirrors 14-PHASE4-API-CONTRACT.md section 2. Admin/agent only,
// same access as the rest of /leads/*.

export async function getLeadConversations(
  leadId: string
): Promise<{ conversationLog: import("./types").ConversationEntry[] }> {
  if (USE_MOCK) {
    const lead = mockState.leads.find((l) => l._id === leadId);
    if (!lead) throw new ApiError(404, { message: "Lead not found" });
    return mockDelay({ conversationLog: lead.conversationLog || [] }, 200);
  }
  return request(`/leads/${leadId}/conversations`);
}

export async function addLeadConversation(
  leadId: string,
  text: string
): Promise<import("./types").ConversationEntry> {
  if (USE_MOCK) {
    const now = new Date().toISOString();
    const entry: import("./types").ConversationEntry = {
      _id: nextId("conv"),
      text,
      createdBy: MOCK_USER.id,
      createdAt: now,
    ...
    const lead = mockState.leads.find((l) => l._id === leadId);
    if (lead) lead.conversationLog = [...(lead.conversationLog || []), entry];
...
export async function updateLeadConversation(
  ...
  if (USE_MOCK) {
    const lead = mockState.leads.find((l) => l._id === leadId);
    const entry = lead?.conversationLog?.find((c) => c._id === entryId);
    if (!entry) throw new ApiError(404, { message: "Conversation entry not found" });
    entry.text = text;
    entry.edited = true;
    entry.updatedAt = new Date().toISOString();
    return mockDelay(entry);
  }
  return request(`/leads/${leadId}/conversations/${entryId}`, {
    method: "PATCH",
    body: JSON.stringify({ text }),
  });
}

export async function deleteLeadConversation(leadId: string, entryId: string) {
  if (USE_MOCK) {
    const lead = mockState.leads.find((l) => l._id === leadId);
    if (lead)
      lead.conversationLog = (lead.conversationLog || []).filter((c) => c._id !== entryId);
    return mockDelay({ message: "Conversation entry deleted" });
  }
  return request<{ message: string }>(`/leads/${leadId}/conversations/${entryId}`, {
    method: "DELETE",
  });
}

// ---------------- Phase 4: Conversation Log (Projects) ----------------
// Additive — mirrors 14-PHASE4-API-CONTRACT.md section 2. Admin only.
...
export async function getProjectConversations(
  ...
  if (USE_MOCK) {
    const project = mockState.projects.find((p) => p._id === projectId);
    if (!project) throw new ApiError(404, { message: "Project not found" });
    return mockDelay({ conversationLog: project.conversationLog || [] }, 200);
  }
...
export async function addProjectConversation(
  ...
  if (USE_MOCK) {
    ...
    const project = mockState.projects.find((p) => p._id === projectId);
    if (project) project.conversationLog = [...(project.conversationLog || []), entry];
    return mockDelay(entry);
  }
  return request(`/projects/${projectId}/conversations`, {
    method: "POST",
...
export async function updateProjectConversation(
  ...
  if (USE_MOCK) {
    const project = mockState.projects.find((p) => p._id === projectId);
    const entry = project?.conversationLog?.find((c) => c._id === entryId);
    if (!entry) throw new ApiError(404, { message: "Conversation entry not found" });
    entry.text = text;
    entry.edited = true;
    entry.updatedAt = new Date().toISOString();
  ...
  return request(`/projects/${projectId}/conversations/${entryId}`, {
    method: "PATCH",
    body: JSON.stringify({ text }),
  });
}

export async function deleteProjectConversation(projectId: string, entryId: string) {
  if (USE_MOCK) {
    const project = mockState.projects.find((p) => p._id === projectId);
    if (project)
      project.conversationLog = (project.conversationLog || []).filter(
        (c) => c._id !== entryId
      );
    return mockDelay({ message: "Conversation entry deleted" });
  }
  return request<{ message: string }>(`/projects/${projectId}/conversations/${entryId}`, {
    method: "DELETE",
  });
}
